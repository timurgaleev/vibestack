...
**Skills audited:** 52. `/spec` reads the Claude-Code `CLAUDE_PLAN_FILE` env var for plan-mode detection and degrades to "inactive" when absent.
**Spec reference:** [agentskills.io/specification](https://agentskills.io/specification)

This audit verifies vibestack's 52 skills against the Agent Skills open standard,
...
|---|---|---|
| Required `name` field present | 52/52 PASS | — |
| `name` matches directory basename | 52/52 PASS | — |
| Required `description` field present | 52/52 PASS | — |
| Description ≤ 1024 chars (spec limit) | 52/52 PASS | — |
| YAML frontmatter parseable | 52/52 PASS | — |
...
| Skills using `AskUserQuestion` (Claude-specific) | ~52 skills | most of the pack |

**Verdict:** All 52 skills are **spec-compliant for file shape**. Cross-target
