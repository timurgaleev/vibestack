...
(function () {
  ...
  if (!Terminal) {
    console.error('[vibestack terminal] xterm not loaded');
    return;
  }
   ...
   * /pty-session and surfaced back via window.vibestackPtySession so the
   ...
   * exposes the relevant fields on window.vibestackHealth (set below in init()).
   * If terminalPort is missing, the agent isn't ready yet.
   */
  function getHealth() {
    return window.vibestackHealth || {};
  }

  function getServerPort() {
    return window.vibestackServerPort || null;
  }

  function getAuthToken() {
    return window.vibestackAuthToken || null;
  }
  ...
  function startReattachLoop(prevSessionId) {
    ...
    const tick = async () => {
      ...
      if (resp.status === 401) {
        ...
        setState(STATE.IDLE, {
          message: 'Auth invalid — reload the sidebar or restart your vibestack session.',
        });
        return;
      }
      if (!resp.ok) {
  ...
  function openReattachWebSocket(terminalPort, attachToken, sessionId) {
    ...
    try { window.vibestackPtySession = currentSessionId; } catch {}
    setState(STATE.LIVE);
    ensureXterm();
    nextBinaryIsReplay = false;
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`vibestack-pty.${attachToken}`]);
    ...
    ws.addEventListener('error', (err) => {
      console.error('[vibestack terminal] reattach ws error', err);
    });
  }
   ...
   * derived input MUST be pre-scanned via window.vibestackScanForPTYInject()
   ...
   * sync `const ok = vibestackInjectToTerminal?.(text)` pattern. Making the
   ...
   */
  window.vibestackInjectToTerminal = function (text) {
   ...
   * Closes #1370.
   */
  window.vibestackScanForPTYInject = async function (text, origin) {
  ...
  async function getAuthTokenForScan() {
    if (window.__vibestackPtyScanToken) return window.__vibestackPtyScanToken;
    try {
      const resp = await fetch('http://127.0.0.1:34567/health');
      const body = await resp.json();
      const token = body.AUTH_TOKEN || body.authToken || '';
      if (token) window.__vibestackPtyScanToken = token;
      return token;
    } catch {
      return '';
    }
  ...
  async function connect() {
    ...
    ws.addEventListener('error', (err) => {
      console.error('[vibestack terminal] ws error', err);
    });
  }
  ...
  async function forceRestart() {
    ...
    if (!nextTuple) {
      ...
      try { window.vibestackPtySession = null; } catch {}
      tryAutoConnect();
      return;
    }
    ...
    setState(STATE.LIVE);
    ensureXterm();
    ws = new WebSocket(`ws://127.0.0.1:${terminalPort}/ws`, [`vibestack-pty.${token}`]);
    ws.binaryType = 'arraybuffer';
  ...
  function init() {
    ...
    document.addEventListener('vibestack:tab-state', (ev) => {
      if (!ws || ws.readyState !== WebSocket.OPEN) return;
      try {
        ws.send(JSON.stringify({
          type: 'tabState',
   ...
   * window.vibestackServerPort + window.vibestackAuthToken (which it does as soon
