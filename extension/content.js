/**
 * vibestack browse — content script
 *
...
function showStatusPill(connected, refs) {
  ...
  if (!statusPill) {
    ...
    statusPill.id = 'vibestack-status-pill';
  ...
  statusPill.innerHTML = `<span class="vibestack-pill-dot"></span> vibestack${refText}`;
...
function ensureContainer() {
  ...
  overlayContainer.id = 'vibestack-ref-overlays';
...
function renderRefBadges(refs) {
  ...
  for (const ref of refs) {
    ...
    badge.className = 'vibestack-ref-badge';
...
function renderRefPanel(refs) {
  ...
  panel.className = 'vibestack-ref-panel';

  const header = document.createElement('div');
  header.className = 'vibestack-ref-panel-header';
  header.textContent = `vibestack refs (${refs.length})`;
  header.style.cssText = 'pointer-events: auto; cursor: move;';
  panel.appendChild(header);

  const list = document.createElement('div');
  list.className = 'vibestack-ref-panel-list';
  for (const ref of refs.slice(0, 30)) { // Show max 30 in panel
    const row = document.createElement('div');
    row.className = 'vibestack-ref-panel-row';
    const idSpan = document.createElement('span');
    idSpan.className = 'vibestack-ref-panel-id';
    idSpan.textContent = ref.ref;
    const roleSpan = document.createElement('span');
    roleSpan.className = 'vibestack-ref-panel-role';
    roleSpan.textContent = ref.role;
    const nameSpan = document.createElement('span');
    nameSpan.className = 'vibestack-ref-panel-name';
  ...
  if (refs.length > 30) {
    ...
    more.className = 'vibestack-ref-panel-more';
...
// Do NOT dispatch vibestack-extension-ready here — the extension being loaded
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'sidebarOpened') {
    document.dispatchEvent(new CustomEvent('vibestack-extension-ready'));
    return;
  }
