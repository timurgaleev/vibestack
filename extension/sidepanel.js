...
document.getElementById('chat-cookies-btn').addEventListener('click', async () => {
  ...
  try {
    ...
    console.error('[vibestack sidebar] Failed to open cookie picker:', err.message);
  }
});
...
async function ensureSseSessionCookie() {
  ...
  try {
    ...
    console.warn('[vibestack sidebar] Failed to mint SSE session cookie:', err && err.message);
    return false;
  }
}
...
async function connectSSE() {
  ...
  eventSource.addEventListener('activity', (e) => {
    try { addEntry(JSON.parse(e.data)); } catch (err) {
      console.error('[vibestack sidebar] Failed to parse activity event:', err.message);
    }
  });
  ...
  eventSource.addEventListener('gap', (e) => {
    try {
      ...
      console.error('[vibestack sidebar] Failed to parse gap event:', err.message);
    }
  });
}
...
async function pollMemoryOnce() {
  ...
  try {
    ...
    try { evaluateMemToast(snapshot); } catch (err) {
      console.debug('[vibestack sidebar] mem-toast evaluation failed:', err && err.message);
    }
    ...
    if (elapsed > MEM_POLL_SLOW_THRESHOLD_MS) {
      console.debug('[vibestack sidebar] /memory poll slow/failed:', elapsed, 'ms', err && err.message);
    }
...
async function loadSnoozeState() {
  ...
  try {
    ...
    console.debug('[vibestack sidebar] mem-toast snooze load failed:', err && err.message);
  }
}
...
async function saveSnoozeState() {
  ...
  try {
    ...
    console.debug('[vibestack sidebar] mem-toast snooze save failed:', err && err.message);
  }
}
  ...
  closeBtn.onclick = async () => {
    ...
    for (const id of ids) {
      try {
        ...
        console.warn('[vibestack sidebar] mem-toast closetab failed:', id, err && err.message);
      }
    }
  };
}
  ...
  if (snooze) {
    snooze.addEventListener('click', async () => {
      ...
      try {
        ...
        console.debug('[vibestack sidebar] mem-toast snooze fetch failed:', err && err.message);
      }
      dismissMemToast();
    });
  }
...
async function fetchRefs() {
  ...
  try {
    ...
    console.error('[vibestack sidebar] Failed to fetch refs:', err.message);
  }
}
...
inspectorSendBtn.addEventListener('click', async () => {
  ...
  // vibestackScanForPTYInject is async; vibestackInjectToTerminal stays sync.
  const verdict = await window.vibestackScanForPTYInject?.(message + '\n', 'inspector-send');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[vibestack sidebar] Inspector send BLOCKED by /pty-inject-scan:', verdict.reasons);
    return;
  }
  ...
  const ok = window.vibestackInjectToTerminal?.(message + '\n');
  if (!ok) {
    console.warn('[vibestack sidebar] Inspector send needs an active Terminal session.');
  }
});
...
async function runCleanup(...buttons) {
  ...
  // confirms every call site goes through vibestackScanForPTYInject first.
  const verdict = await window.vibestackScanForPTYInject?.(cleanupPrompt + '\n', 'cleanup-button');
  if (verdict?.verdict === 'BLOCK') {
    console.warn('[vibestack sidebar] Cleanup BLOCKED by /pty-inject-scan:', verdict.reasons);
  ...
  const sent = window.vibestackInjectToTerminal?.(cleanupPrompt + '\n');
  if (!sent) {
    console.warn('[vibestack sidebar] Cleanup needs an active Terminal session.');
  }
...
async function runScreenshot(...buttons) {
  ...
  try {
    ...
    if (!resp.ok) {
      console.warn('[vibestack sidebar] Screenshot failed:', text);
    } else {
      console.log('[vibestack sidebar] Screenshot:', text);
    }
  } catch (err) {
    console.error('[vibestack sidebar] Screenshot error:', err.message);
  } finally {
  ...
  try {
    ...
    inspectorSSE.addEventListener('inspectResult', (e) => {
      try {
        ...
        console.error('[vibestack sidebar] Failed to parse inspectResult:', err.message);
      }
    });
    ...
    console.debug('[vibestack sidebar] Inspector SSE not available:', err.message);
  }
}
...
function updateConnection(url, token) {
  ...
  if (url) {
    try { window.vibestackServerPort = parseInt(new URL(url).port, 10); } catch {}
    window.vibestackAuthToken = token || null;
  } else {
    window.vibestackServerPort = null;
    window.vibestackAuthToken = null;
  }
  if (url) {
...
document.getElementById('conn-copy').addEventListener('click', () => {
  navigator.clipboard.writeText('/open-vibestack-browser').then(() => {
    const btn = document.getElementById('conn-copy');
    btn.textContent = 'copied!';
    setTimeout(() => { btn.textContent = '/open-vibestack-browser'; }, 2000);
  });
});
...
chrome.runtime.onMessage.addListener((msg) => {
  ...
  if (msg.type === 'browserTabState') {
    document.dispatchEvent(new CustomEvent('vibestack:tab-state', {
...
window.addEventListener('pagehide', () => {
  const sessionId = window.vibestackPtySession;
  const authToken = window.vibestackAuthToken;
  const port = window.vibestackServerPort;
