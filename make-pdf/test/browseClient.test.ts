...
describe("resolveBrowseBin", () => {
  test("throws BrowseClientError with setup hint when nothing is found", () => {
    ...
    try {
      withEnv(
        {
          VIBE_BROWSE_BIN: "/nonexistent/vibestack-browse-bin",
    ...
    if (thrown) {
      ...
      expect((thrown as BrowseClientError).message).toContain("./install");
      expect((thrown as BrowseClientError).message).toContain("VIBE_BROWSE_BIN");
  ...
  test("honors VIBE_BROWSE_BIN when it points at a real executable", () => {
    const resolved = withEnv({ VIBE_BROWSE_BIN: REAL_EXE }, () => resolveBrowseBin());
    expect(resolved).toBe(REAL_EXE);
  });

  test("honors BROWSE_BIN as a back-compat alias", () => {
    const resolved = withEnv(
      { VIBE_BROWSE_BIN: undefined, BROWSE_BIN: REAL_EXE },
      () => resolveBrowseBin(),
    );
    expect(resolved).toBe(REAL_EXE);
  });

  test("VIBE_BROWSE_BIN takes precedence over BROWSE_BIN", () => {
    const resolved = withEnv(
      { VIBE_BROWSE_BIN: REAL_EXE, BROWSE_BIN: "/nonexistent/legacy" },
      () => resolveBrowseBin(),
    );
    expect(resolved).toBe(REAL_EXE);
  });
