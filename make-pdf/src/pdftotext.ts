 ...
 *   1. $VIBESTACK_PDFTOTEXT_BIN env override (preferred, matches v1.24 VIBESTACK_*_BIN pattern)
 ...
 *      and guessing causes false positives. Set VIBESTACK_PDFTOTEXT_BIN explicitly.
...
export function resolvePdftotext(env: NodeJS.ProcessEnv = process.env): PdftotextInfo {
  // 1 + 2: env overrides (VIBESTACK_PDFTOTEXT_BIN preferred, PDFTOTEXT_BIN back-compat).
  const overrideRaw = env.VIBESTACK_PDFTOTEXT_BIN ?? env.PDFTOTEXT_BIN;
  ...
  // positives. Windows users set VIBESTACK_PDFTOTEXT_BIN explicitly.
  ...
  throw new PdftotextUnavailableError([
    ...
    "",
    "Or set VIBESTACK_PDFTOTEXT_BIN to an explicit path:",
    process.platform === "win32"
      ? '  setx VIBESTACK_PDFTOTEXT_BIN "C:\\path\\to\\pdftotext.exe"'
      : "  export VIBESTACK_PDFTOTEXT_BIN=/path/to/pdftotext",
  ].join("\n"));
}
 ...
 *   1. $VIBESTACK_<TOOL>_BIN env override (e.g. VIBESTACK_PDFFONTS_BIN)
