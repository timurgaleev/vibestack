 ...
 *   1. $VIBE_BROWSE_BIN env override (preferred, matches v1.24 VIBESTACK_*_BIN pattern)
 *   2. $BROWSE_BIN env override (back-compat alias)
 *   3. sibling dir: repo checkout ../../browse/bin/browse (bun launcher)
 *   4. ~/.claude/skills/browse/bin/vibe-browse (installed skill launcher)
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  // 1 + 2: env overrides (VIBE_BROWSE_BIN preferred, BROWSE_BIN back-compat).
  const overrideRaw = env.VIBE_BROWSE_BIN ?? env.BROWSE_BIN;
  const override = resolveOverride(overrideRaw, env);
  if (override) return override;

  // 3: sibling — the repo's bun launcher. Two contexts:
  //   bun-run:  make-pdf/src/cli.ts → ../../browse/bin/browse (import.meta.url
  //             is real on disk).
  //   compiled: import.meta.url lives in bun's virtual /$bunfs — useless. The
  //             binary sits at <root>/make-pdf/dist/pdf, so resolve from
  //             process.execPath instead (argv[0] is the literal "bun").
  const selfDir = path.dirname(new URL(import.meta.url).pathname);
  const execDir = path.dirname(process.execPath);
  const siblingCandidates = [
    path.resolve(selfDir, "../../browse/bin/browse"),
    path.resolve(execDir, "../../browse/bin/browse"),
    path.resolve(execDir, "../browse/bin/browse"),
  ];
  ...
  // 4: global install — the installed browse skill's launcher.
  const home = os.homedir();
  const globalPath = path.join(home, ".claude/skills/browse/bin/vibe-browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "",
      "make-pdf needs browse (the vibestack Chromium daemon) to render PDFs.",
      "Tried:",
      `  - $VIBE_BROWSE_BIN (${env.VIBE_BROWSE_BIN || "unset"})`,
      `  - $BROWSE_BIN (${env.BROWSE_BIN || "unset"})`,
      `  - sibling: ${siblingCandidates.join(", ")}`,
      `  - global: ${globalPath}`,
      "  - PATH: `browse`",
      "",
      "To fix: install the pack from the vibestack repo:",
      "  cd ~/data/vibestack && ./install",
      "",
      "Or set VIBE_BROWSE_BIN explicitly:",
      process.platform === "win32"
        ? '  setx VIBE_BROWSE_BIN "C:\\path\\to\\browse.exe"'
        : "  export VIBE_BROWSE_BIN=/path/to/browse",
    ].join("\n"),
  );
}
