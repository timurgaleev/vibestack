 ...
 *
 * /skillify stages a candidate skill into ~/.vibestack/.tmp/skillify-<spawnId>/,
 ...
 *   <tmpRoot>/.vibestack/.tmp/skillify-<spawnId>/<name>/
 *
...
export function stageSkill(opts: StageSkillOptions): string {
  ...
  const tmpRoot = opts.tmpRoot ?? path.join(os.homedir(), '.vibestack', '.tmp');
