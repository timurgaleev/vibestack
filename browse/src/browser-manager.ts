...
/**
 * Detect whether VIBESTACK_CHROMIUM_PATH points at a custom Chromium build that
 * already bakes the vibestack extension in as a component extension (e.g.,
 * vibestack Browser.app / vibestack-browser). Passing --load-extension against such a
 * binary triggers a ServiceWorkerState::SetWorkerId DCHECK because two
 * copies of the same service worker try to register.
 *
 * Resolution:
 *   1. VIBESTACK_CHROMIUM_KIND === 'custom-extension-baked' (preferred, explicit)
 *   2. VIBESTACK_CHROMIUM_PATH path substring contains 'vibestack-browser' or 'vibestack-browser'
 *      (fallback for callers that only set the path)
 */
export function isCustomChromium(): boolean {
  if (process.env.VIBESTACK_CHROMIUM_KIND === 'custom-extension-baked') return true;
  const p = process.env.VIBESTACK_CHROMIUM_PATH || '';
  return p.includes('vibestack-browser') || p.includes('vibestack-browser');
}
...
export function shouldEnableChromiumSandbox(): boolean {
  ...
  // so /qa hangs without --no-sandbox). Setting VIBESTACK_CHROMIUM_NO_SANDBOX=1
  // forces the sandbox off without changing the default for everyone else.
  // See #1562.
  if (process.env.VIBESTACK_CHROMIUM_NO_SANDBOX === '1') return false;
 ...
 *
 * Process supervisors (vibestack-browser's gbd HealthMonitor in cmd/gbd/health.go)
...
export async function handleChromiumDisconnect(browser: Browser | null): Promise<void> {
  ...
  console.error('[browse] Console/network logs flushed to .vibestack/browse-*.log');
  process.exit(1);
}
...
export class BrowserManager {
  ...
  // pipeline so process supervisors (vibestack-browser's gbd) read the right signal.
  ...
  /**
   * Find the vibestack Chrome extension directory.
  ...
  private findExtensionPath(): string | null {
    ...
    const candidates = [
      // Explicit override via env var (used by vibestack Browser.app bundle)
      process.env.BROWSE_EXTENSIONS_DIR || '',
      // Relative to this source file (dev mode: browse/src/ -> ../../extension)
      path.resolve(__dirname, '..', '..', 'extension'),
      // Global vibestack install
      path.join(process.env.HOME || '', '.claude', 'skills', 'vibestack', 'extension'),
      // Git repo root (detected via BROWSE_STATE_FILE location)
      (() => {
        const stateFile = process.env.BROWSE_STATE_FILE || '';
        if (stateFile) {
          const repoRoot = path.resolve(path.dirname(stateFile), '..');
          return path.join(repoRoot, '.claude', 'skills', 'vibestack', 'extension');
        }
        return '';
      })(),
    ].filter(Boolean);
  ...
  async launch() {
    ...
    const { STEALTH_LAUNCH_ARGS, buildvibestackLaunchArgs } = await import('./stealth');
    const launchArgs: string[] = [...STEALTH_LAUNCH_ARGS, ...buildvibestackLaunchArgs()];
    ...
    if (extensionsDir) {
      ...
      // already bakes the extension in (e.g., vibestack-browser / vibestack Browser.app).
    ...
    // Process supervisors (vibestack-browser's gbd) consume our exit code: code 0
  ...
  // ─── Headed Mode ─────────────────────────────────────────────
  /**
   * Launch Playwright's bundled Chromium in headed mode with the vibestack
  ...
  async launchHeaded(authToken?: string): Promise<void> {
    ...
    // Find the vibestack extension directory for auto-loading
    ...
    const launchArgs = [
      ...
      ...STEALTH_LAUNCH_ARGS,
      // vibestack Pack 1: per-install hardware/GPU/UA-CH overrides for the
      // C++ patches in vibestack-browser's Chromium build. Each switch is a no-op
    ...
    if (extensionPath) {
      ...
      // (vibestack-browser / vibestack Browser.app). Loading it twice causes a
      ...
      // the extension is component-baked — it reads ~/.vibestack/.auth.json at
      // startup to learn how to call the daemon).
      // Write to ~/.vibestack/.auth.json (not the extension dir, which may be read-only
      // in .app bundles and breaks codesigning).
      if (authToken) {
        const fs = require('fs');
        const path = require('path');
        const vibestackDir = path.join(process.env.HOME || '/tmp', '.vibestack');
        mkdirSecure(vibestackDir);
        const authFile = path.join(vibestackDir, '.auth.json');
        try {
    ...
    // too. Safe under external coordination: gbd.lock for vibestack-browser,
    // single-instance CLI check for vibestack.
    cleanSingletonLocks(userDataDir);

    // Support custom Chromium binary via VIBESTACK_CHROMIUM_PATH env var.
    // Used by vibestack Browser.app to point at the bundled Chromium.
    const executablePath = process.env.VIBESTACK_CHROMIUM_PATH || undefined;

    // Rebrand Chromium → vibestack Browser in macOS menu bar / Dock / Cmd+Tab.
      ...
      if (fs.existsSync(chromePlist)) {
        ...
        if (plistContent.includes('Google Chrome for Testing')) {
          const patched = plistContent
            .replace(/Google Chrome for Testing/g, 'vibestack Browser');
        ...
        const iconCandidates = [
          ...
          path.join(process.env.HOME || '', '.claude', 'skills', 'vibestack', 'scripts', 'app', 'icon.icns'), // global install
        ];
    ...
    // the previous "vibestackBrowser" branding suffix was itself a high-entropy
    ...
    // "vibestack-browser." Removing it resolves the "looks like Chrome but identifies
    // as vibestackBrowser" contradiction codex flagged.
    ...
    const indicatorScript = () => {
      const injectIndicator = () => {
        if (document.getElementById('vibestack-ctrl')) return;

        const topLine = document.createElement('div');
        topLine.id = 'vibestack-ctrl';
        topLine.style.cssText = `
          position: fixed; top: 0; left: 0; right: 0; height: 2px;
          background: linear-gradient(90deg, #F59E0B, #FBBF24, #F59E0B);
          background-size: 200% 100%;
          animation: vibestack-shimmer 3s linear infinite;
        ...
        style.textContent = `
          @keyframes vibestack-shimmer {
            0% { background-position: 200% 0; }
            100% { background-position: -200% 0; }
          }
          @media (prefers-reduced-motion: reduce) {
            #vibestack-ctrl { animation: none !important; }
          }
        `;
    ...
    // supervisors (vibestack-browser's gbd) treat it as user intent and skip the
  ...
  async setDeviceScaleFactor(scale: number, width: number, height: number): Promise<string | null> {
    ...
    if (scale < 1 || scale > 3) {
      throw new Error(`viewport --scale: value must be between 1 and 3 (vibestack policy cap), got ${scale}`);
    }
    if (this.connectionMode === 'headed') {
  ...
  async handoff(message: string): Promise<string> {
    ...
    try {
      ...
      const launchArgs: string[] = ['--hide-crash-restore-bubble', ...STEALTH_LAUNCH_ARGS, ...buildvibestackLaunchArgs()];
      if (extensionPath) {
        launchArgs.push(`--disable-extensions-except=${extensionPath}`);
        launchArgs.push(`--load-extension=${extensionPath}`);
      ...
      const userDataDir = path.join(process.env.HOME || '/tmp', '.vibestack', 'chromium-profile');
      fs.mkdirSync(userDataDir, { recursive: true });
  ...
  private wirePageEvents(page: Page) {
    ...
    // pages, the primary Bun-side accelerant on the vibestack-browser-OOM
