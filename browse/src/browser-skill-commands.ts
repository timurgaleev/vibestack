 ...
 * and passes it via VIBESTACK_SKILL_TOKEN. The skill never sees the daemon root
 ...
 *    VIBESTACK_PORT and VIBESTACK_SKILL_TOKEN are always set.
 ...
 * Includes: minimal PATH, locale, terminal type. Skills get VIBESTACK_PORT +
 * VIBESTACK_SKILL_TOKEN injected separately.
 */
...
export function buildSpawnEnv(opts: BuildEnvOptions): Record<string, string> {
  ...
  if (opts.trusted) {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      ...
      if (k === 'VIBESTACK_TOKEN') continue; // never propagate root token
      out[k] = v;
    }
  ...
  // intentionally — e.g. an internal-tool skill — but we still strip VIBESTACK_TOKEN
  // above.)
  if (!opts.trusted) {
  ...
  out.VIBESTACK_PORT = String(opts.port);
  out.VIBESTACK_SKILL_TOKEN = opts.skillToken;
