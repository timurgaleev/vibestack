/**
 * Cross-platform file permission restriction for sensitive vibestack state.
 *
 * Why this exists
 * ----------------
 * POSIX mode bits (`0o600` for files, `0o700` for dirs) are how vibestack marks
 ...
 * install, sensitive vibestack state files inherit whatever ACL the parent
...
function warnIcaclsFailure(fsPath: string, err: unknown): void {
  ...
  console.warn(
    `[vibestack] Failed to restrict Windows ACL on ${fsPath}: ${msg}\n` +
