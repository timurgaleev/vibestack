/**
 * Terminal Agent — PTY-backed Claude Code terminal for the vibestack browser
 * sidebar. Translates the phoenix vibestack-browser PTY (cmd/gbd/terminal.go) into
...
const STATE_FILE = process.env.BROWSE_STATE_FILE || path.join(process.env.HOME || '/tmp', '.vibestack', 'browse.json');
...
const KEEPALIVE_INTERVAL_MS = parseInt(
  process.env.VIBESTACK_PTY_KEEPALIVE_INTERVAL_MS || '25000',
  10,
);
...
const RING_BUFFER_MAX_BYTES = parseInt(
  process.env.VIBESTACK_PTY_RING_BUFFER_BYTES || `${1024 * 1024}`,
  10,
);
...
const DETACH_WINDOW_MS = parseInt(
  process.env.VIBESTACK_PTY_DETACH_WINDOW_MS || '60000',
  10,
);
 ...
 *      (updated continuously by the vibestack browser extension).
...
function buildTabAwarenessHint(stateDir: string): string {
  ...
  return [
    'You are running inside the vibestack browser sidebar with live access to the user\'s browser tabs.',
    '',
...
function spawnClaude(cols: number, rows: number, onData: (chunk: Buffer) => void) {
  ...
  // headed-mode browser; BROWSE_NO_AUTOSTART prevents claude's vibestack
...
function buildServer() {
  return Bun.serve({
    ...
    fetch(req, server) {
      ...
      //         - Cookie vibestack_pty (works for non-browser callers and
      ...
      if (url.pathname === '/ws') {
        ...
        // with a `vibestack-pty.` prefix (which we strip). Browsers send a
        ...
        for (const raw of protoHeader.split(',').map(s => s.trim()).filter(Boolean)) {
          const candidate = raw.startsWith('vibestack-pty.') ? raw.slice('vibestack-pty.'.length) : raw;
        ...
        // Fallback: Cookie vibestack_pty (legacy / non-browser callers).
        if (!token) {
          const cookieHeader = req.headers.get('cookie') || '';
          for (const part of cookieHeader.split(';')) {
            const [name, ...rest] = part.trim().split('=');
            if (name === 'vibestack_pty') {
...
function main() {
  ...
  // sibling vibestack sessions. Callers (cli.ts spawn site, server.ts
