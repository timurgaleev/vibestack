/**
 * find-browse — locate the vibestack browse binary.
 *
...
export function locateBinary(): string | null {
  ...
  if (root) {
    for (const m of markers) {
      const local = join(root, m, 'skills', 'vibestack', 'browse', 'dist', 'browse');
    ...
    // - vibestack repo dev workflow before `./setup` runs
  ...
  // Global fallback
  for (const m of markers) {
    const global = join(home, m, 'skills', 'vibestack', 'browse', 'dist', 'browse');
