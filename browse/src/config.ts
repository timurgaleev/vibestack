 ...
 *   2. git rev-parse --show-toplevel → projectDir/.vibestack/
...
export function resolveConfig(
  ...
  if (env.BROWSE_STATE_FILE) {
    ...
    projectDir = path.dirname(stateDir); // parent of .vibestack/
  } else {
    projectDir = getGitRoot() || process.cwd();
    stateDir = path.join(projectDir, '.vibestack');
...
/**
 * Create the .vibestack/ state directory if it doesn't exist.
...
/**
 * Resolve the vibestack home directory.
 *
 * Honors the existing convention used by telemetry.ts and domain-skills.ts:
 *   1. VIBESTACK_HOME env (explicit override)
 *   2. $HOME/.vibestack (default)
 */
export function resolvevibestackHome(): string {
 ...
 *   2. CHROMIUM_PROFILE env (used by vibestack-browser's gbd per-workspace)
 *   3. <resolvevibestackHome()>/chromium-profile (default)
 */
export function resolveChromiumProfile(explicit?: string): string {
  if (explicit && explicit.length > 0) return explicit;
  const env = process.env.CHROMIUM_PROFILE;
  if (env && env.length > 0) return env;
  return path.join(resolvevibestackHome(), 'chromium-profile');
}
