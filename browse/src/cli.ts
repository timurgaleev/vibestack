/**
 * vibestack CLI — thin wrapper that talks to the persistent server
 *
 * Flow:
 *   1. Read .vibestack/browse.json for port + token
...
function cleanupLegacyState(): void {
  // No legacy state on Windows — /tmp and `ps` don't exist, and vibestack
...
async function ensureServer(flags?: GlobalFlags): Promise<ServerState> {
  ...
  if (process.env.BROWSE_NO_AUTOSTART === '1') {
    ...
    console.error('[browse] The headed browser may have been closed. Run /open-vibestack-browser to restart.');
    process.exit(1);
  }
  ...
  if (state && state.mode === 'headed' && isProcessAlive(state.pid)) {
    ...
    console.error(`[browse] Run '/open-vibestack-browser' to restart.`);
    process.exit(1);
  }
...
async function handlePairAgent(state: ServerState, args: string[]): Promise<void> {
  ...
  if (localHost) {
    try {
      ...
      let globalRoot = `.${localHost}/skills/vibestack`;
      try {
...
async function main() {
  ...
  if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
    console.log(`vibestack browse — Fast headless browser for AI coding agents
...
Refs:           After 'snapshot', use @e1, @e2... as selectors:
  ...
  if (command === 'connect') {
    ...
    const SUPERVISOR_TICK_MS = parseInt(
      process.env.VIBESTACK_SUPERVISOR_TICK_MS || '30000',
      10,
    );
    const SUPERVISOR_GUARD_WINDOW_MS = 5 * 60_000;
    const SUPERVISOR_GUARD_MAX = 5;
    const SUPERVISOR_BACKOFF_MS = (process.env.VIBESTACK_SUPERVISOR_BACKOFF || '1000,2000,4000,8000,30000')
