/**
 * Stealth init script — Layer C of vibestack-browser's anti-detection plan.
 *
 ...
 * VIBESTACK_STEALTH=extended, off by default) DOES fake plugins — that mode
 ...
 *      detectors flag.
 *   4. Report per-install hardware values via VIBESTACK_HW_CONCURRENCY /
 *      VIBESTACK_DEVICE_MEMORY env vars (set by gbd at startup via
 ...
 * on macOS), passed through the VIBESTACK_* env vars. Each field falls
...
// the UA-CH cmdline switch in buildvibestackLaunchArgs (which reads VIBESTACK_PLATFORM
...
export function readHostProfile(): HostProfile {
  ...
  const concurrency = Number(env.VIBESTACK_HW_CONCURRENCY);
  const memory = Number(env.VIBESTACK_DEVICE_MEMORY);
  return {
...
export function buildStealthScript(hw: HostProfile): string {
  ...
  // ──── Per-install hardware values from VIBESTACK_* env (T2) ────
  // gbd's host_profile.go fed real host values via cmdline env. Reporting
  // those (not hardcoded defaults) avoids the cross-user vibestack-browser
  // fingerprint cluster.
  try {
...
function extendedModeEnabled(): boolean {
  const v = process.env.VIBESTACK_STEALTH;
 ...
 *   3. EXTENDED_STEALTH_SCRIPT — only when VIBESTACK_STEALTH=extended (off by
 ...
 * disagree with the env-driven GPU spoof in buildvibestackLaunchArgs on non-Apple
...
/**
 * Build the `--vibestack-*=` cmdline switches that the Pack 1 Chromium
 * patches read (webgl-vendor-spoof, ua-client-hints-stealth, worker-
 * navigator-stealth). Values come from the VIBESTACK_* env vars that
 ...
 * on builds that DO NOT have the C++ patches applied (vibestack-browser
 * pre-Pack-1) and on hosts where gbd hasn't yet populated some
 * fields (legacy installs).
 *
 * TRUSTED-SOURCE ONLY. These VIBESTACK_* values are populated by gbd from
 ...
 *   VIBESTACK_GPU_VENDOR        → --vibestack-gpu-vendor        → webgl-vendor-spoof.patch
 *   VIBESTACK_GPU_RENDERER      → --vibestack-gpu-renderer      → webgl-vendor-spoof.patch
 *   VIBESTACK_PLATFORM          → --vibestack-ua-platform       → ua-client-hints-stealth.patch
 *                              (maps MacARM/MacIntel → "macOS")
 *   VIBESTACK_GPU_CHIPSET       → --vibestack-ua-model          → ua-client-hints-stealth.patch
 *   VIBESTACK_HW_CONCURRENCY    → --vibestack-hw-concurrency    → worker-navigator-stealth.patch
 *   VIBESTACK_DEVICE_MEMORY     → --vibestack-device-memory     → worker-navigator-stealth.patch
 */
export function buildvibestackLaunchArgs(): string[] {
  const env = (globalThis as any).process?.env ?? {};
  const args: string[] = [];

  const vendor = env.VIBESTACK_GPU_VENDOR;
  if (vendor) args.push(`--vibestack-gpu-vendor=${vendor}`);

  const renderer = env.VIBESTACK_GPU_RENDERER;
  if (renderer) args.push(`--vibestack-gpu-renderer=${renderer}`);
  ...
  const platform = env.VIBESTACK_PLATFORM;
  if (platform === 'MacARM' || platform === 'MacIntel') {
    args.push('--vibestack-ua-platform=macOS');
  } else if (platform === 'Win32') {
    args.push('--vibestack-ua-platform=Windows');
  } else if (platform && platform.startsWith('Linux')) {
    args.push('--vibestack-ua-platform=Linux');
  }

  const chipset = env.VIBESTACK_GPU_CHIPSET;
  if (chipset) args.push(`--vibestack-ua-model=${chipset}`);

  const hw = env.VIBESTACK_HW_CONCURRENCY;
  if (hw) args.push(`--vibestack-hw-concurrency=${hw}`);

  const memory = env.VIBESTACK_DEVICE_MEMORY;
  if (memory) args.push(`--vibestack-device-memory=${memory}`);
  ...
  //
  // OPT-IN (off by default): only emitted when VIBESTACK_CDP_STEALTH is
  // on/1/true. This switch is read by a C++ patch that only exists in
  // vibestack-browser builds; gbd opts in by exporting VIBESTACK_CDP_STEALTH=on. Stock
  ...
  // unless VIBESTACK_CDP_STEALTH=off, which contradicted this very comment.)
  const cdpStealth = env.VIBESTACK_CDP_STEALTH;
  if (cdpStealth === 'on' || cdpStealth === '1' || cdpStealth === 'true') {
    args.push('--vibestack-suppress-prepare-stack-trace');
  }
