 ...
 *
 * Scope:
 *   - per-project: ~/.vibestack/projects/<slug>/learnings.jsonl
 *   - global:      ~/.vibestack/global-domain-skills.jsonl
 *
...
function globalFile(): string {
  return path.join(vibestackHome(), 'global-domain-skills.jsonl');
}

function projectFile(slug: string): string {
  return path.join(vibestackHome(), 'projects', slug, 'learnings.jsonl');
}
