/**
 * vibestack browse server — persistent Chromium daemon
 *
 * Architecture:
 ...
 *
 * State:
 *   State file: <project-root>/.vibestack/browse.json (set via BROWSE_STATE_FILE env)
 *   Log files:  <project-root>/.vibestack/browse-{console,network,dialog}.log
...
// (vibestack-browser's gbd daemon spawn) can pre-allocate the token and hand it to
...
/**
 * Factory contract for embedders (vibestack-browser phoenix overlay).
 *
 * Today the CLI calls `start()` which reads env vars and binds Bun.serve
 * itself. Embedders building on this server as a submodule (vibestack-browser's
 ...
 * `/Users/timurgaleev/.claude/plans/system-instruction-you-are-working-swirling-fountain.md`
 * Part 1.
 */
export interface ServerConfig {
  ...
  startTime: number;
  /**
   * Overlay hook. Runs AFTER vibestack resolves auth and BEFORE route dispatch.
   * Invalid tokens are auto-rejected at the vibestack layer (401 returned
   * before hook fires), so the hook only ever sees valid TokenInfo or null
   * (no token presented). Returning a Response short-circuits vibestack
   * dispatch; returning null falls through.
   */
  beforeRoute?: (req: Request, surface: Surface, auth: TokenInfo | null) => Promise<Response | null>;
  /**
   * Whether vibestack owns the lifecycle of the terminal-agent process and its
   ...
   *      matched sibling vibestack sessions on the same host — see
   ...
   *
   * This is correct for vibestack's CLI path, which spawns `terminal-agent.ts` as
   * the producer of those files (see cli.ts:1037-1063).
   *
   * Embedders (vibestack-browser phoenix overlay, future hosts) that run their own PTY
...
/**
 * Build a ServerConfig-shaped object from process.env. Used by vibestack's
...
/**
 * Read ngrok authtoken from env var, ~/.vibestack/ngrok.env, or ngrok's native
...
function resolveNgrokAuthtoken(): string | null {
  ...
  const ngrokEnvPath = path.join(home, '.vibestack', 'ngrok.env');
...
function generateHelpText(): string {
  ...
  const lines = ['vibestack browse — headless browser for AI agents', '', 'Commands:'];
...
// so this branch is the normal path for /open-vibestack-browser.
...
if (BROWSE_PARENT_PID > 0 && !IS_HEADED_WATCHDOG) {
  ...
  setInterval(() => {
    try {
      ...
      //    /pair-agent or /open-vibestack-browser sessions.
...
// Without this, embedders like vibestack-browser hit the dead module-level instance
...
// macOS) so process supervisors like vibestack-browser's gbd skip the restart loop;
...
//
// Gated on `import.meta.main` so embedders (vibestack-browser phoenix) that import
// server.ts as a submodule can register their own signal handlers without
// fighting with vibestack's. CLI path is unchanged.
if (import.meta.main) {
 ...
 *
 * Exported so embedders (vibestack-browser phoenix overlay) can call it
 ...
 */
/**
 * Build a request handler set for the browse daemon. Embedders (vibestack-browser
...
export function buildFetchHandler(cfg: ServerConfig): ServerHandle {
  ...
  // to vibestack-owns. Matches the "default-true preserves CLI bit-for-bit"
  ...
  // their own PTY server (vibestack-browser phoenix overlay) must not be auto-respawned
  ...
  const AGENT_WATCHDOG_TICK_MS = parseInt(
    process.env.VIBESTACK_AGENT_WATCHDOG_TICK_MS || '60000',
    10,
  );
  ...
  async function shutdown(exitCode: number = 0) {
    ...
    if (ownsTerminalAgent) {
      ...
      // terminal-agent\.ts` regex teardown which matched sibling vibestack
  ...
  // instead of overwriting it: vibestack-browser may have set its own onDisconnect
  ...
  // block vibestack shutdown — defensive symmetry with the safeUnlinkQuiet /
  ...
  const makeFetchHandler = (surface: Surface) => async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === '/connect' && req.method === 'GET') {
      ...
      // Welcome page — served when vibestack Browser launches in headed mode
      if (url.pathname === '/welcome') {
        const welcomePath = (() => {
          // Gate VIBESTACK_SLUG on a strict regex BEFORE interpolating it into
          // the filesystem path. Without this, a slug like "../../etc/passwd"
          // would resolve to ~/.vibestack/projects/../../etc/passwd/... — path
          ...
          const rawSlug = process.env.VIBESTACK_SLUG || 'unknown';
          const slug = /^[a-z0-9_-]+$/.test(rawSlug) ? rawSlug : 'unknown';
          const homeDir = process.env.HOME || process.env.USERPROFILE || '/tmp';
          const projectWelcome = `${homeDir}/.vibestack/projects/${slug}/designs/welcome-page-20260331/finalized.html`;
          if (fs.existsSync(projectWelcome)) return projectWelcome;
          // Fallback: built-in welcome page from vibestack install.  Reject
          // SKILL_ROOT values containing '..' for the same defense-in-depth
          // reason as the VIBESTACK_SLUG regex above.  Not exploitable today
          // (env set at install time), but the gate is one check.
          const rawSkillRoot = process.env.VIBESTACK_SKILL_ROOT || `${homeDir}/.claude/skills/vibestack`;
        ...
        return new Response(
          `<!DOCTYPE html><html><head><title>vibestack Browser</title>
          ...
          <body><div class="msg"><div class="gold">◈</div><p>vibestack Browser ready.</p><p style="font-size:.85em">Waiting for commands from Claude Code.</p></div></body></html>`,
      ...
      // extension's vibestackInjectToTerminal callers. The extension routes
      ...
      if (url.pathname === '/tunnel/start' && req.method === 'POST') {
        ...
        // 1) Resolve ngrok authtoken from env / .vibestack / native config
...
// (vibestack-browser phoenix overlay) import { start, buildFetchHandler, ... }
