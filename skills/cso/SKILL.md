...
**Under `--diff`:** the account has no diff to constrain, so this phase runs only when the branch diff touches AWS configuration — a Terraform file declaring an `aws` provider or an `aws_*` resource, a CDK app file, an IAM policy document, or a deploy config naming AWS. If it does, run the phase in full against the whole account and say in the report that this phase alone was not limited to the branch. If it does not, print `AWS posture: skipped (--diff, no AWS configuration changed on this branch)` and omit `"5b"` from `phases_run`. Check this before the credential gate below.
...
| S3 public access | `aws s3control get-public-access-block --account-id <id>`, then `aws s3api list-buckets` and per bucket `aws s3api get-public-access-block --bucket <b>`, `aws s3api get-bucket-policy-status --bucket <b>`, and `aws s3api list-objects-v2 --bucket <b> --max-keys 100 --query 'Contents[].Key'` | Account-level block missing or any of its four flags false → MEDIUM. Bucket with `IsPublic: true` → MEDIUM, raised to CRITICAL only on the object evidence below |
...
**Public bucket severity needs object evidence.** The public-access and policy-status calls say a bucket is reachable, not what is in it, so severity turns on the sampled key listing. Keys that are all static site assets (`.html`, `.css`, `.js`, images, fonts) together with an `aws s3api get-bucket-website --bucket <b>` that succeeds → MEDIUM. Keys showing anything else — backups, database dumps, exports, user uploads, documents, archives → CRITICAL, and the finding quotes the keys that decided it. If the listing is denied, empty, or the keys do not settle the question, the finding stays MEDIUM and its description opens with "Classification pending manual confirmation — the object sample could not show what this bucket holds"; the public flag on its own never carries CRITICAL. The title is the same in all three cases, so the Phase 13 fingerprint stays stable when a later run can read the sample.

**Severity:** CRITICAL for root access keys present / `"*"`-on-`"*"` policy attached to an IAM user / public bucket whose sampled keys show non-static content. HIGH for root or user without MFA / no logging multi-region trail with validation / 0.0.0.0/0 or ::/0 on 22, 3389, or all ports. MEDIUM for everything else in the table.

**FP rules:** A bucket that serves a static site (`aws s3api get-bucket-website --bucket <b>` succeeds) and whose sampled keys are all static assets is MEDIUM, not CRITICAL — still report it so the owner confirms. A security group with no attached network interfaces (`aws ec2 describe-network-interfaces --filters Name=group-id,Values=<sg>` empty) drops one level. Access keys that an external system rotates (name or tag says so): state the rotation source in the finding instead of flagging age alone. Sandbox or personal accounts (caller ARN contains `sandbox`, `dev`, or `personal`) get the same checks, and the finding says which account it is.
