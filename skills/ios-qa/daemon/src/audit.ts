...
export function defaultAuditPath(): string {
  return process.env.VIBESTACK_IOS_AUDIT_PATH
    ?? join(homedir(), '.vibestack', 'security', 'ios-qa-audit.jsonl');
}

export function defaultAttemptsPath(): string {
  return process.env.VIBESTACK_IOS_ATTEMPTS_PATH
    ?? join(homedir(), '.vibestack', 'security', 'attempts.jsonl');
}
...
async function loadDeviceSalt(): Promise<string> {
  ...
  const path = join(homedir(), '.vibestack', 'security', 'device-salt');
  try {
