...
//
// Invoked from bin/vibe-ios-qa-mint.
...
function printHelp() {
  const help = `vibe-ios-qa-mint — manage the tailnet allowlist for remote iOS QA agents

USAGE
  vibe-ios-qa-mint grant  --remote <identity> [--capability <tier>] [--ttl <seconds>] [--note <text>]
  vibe-ios-qa-mint revoke --remote <identity>
...
EXAMPLES
  ...
  vibe-ios-qa-mint grant --remote 'tag:ci' --capability mutate --ttl 86400 --note 'nightly run'
  vibe-ios-qa-mint revoke --remote 'alice@example.com'
  vibe-ios-qa-mint list

The allowlist lives at ~/.vibestack/ios-qa-allowlist.json (mode 0600). The daemon's
...
if (import.meta.main) {
  main().catch((err) => {
    process.stderr.write(`vibe-ios-qa-mint: ${(err as Error).message}\n`);
    process.exit(1);
  });
}
