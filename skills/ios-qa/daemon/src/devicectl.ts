...
export async function getDeviceTunnelIPv6(
  ...
  // ".coredevice.local" suffix. Apple normalizes e.g. "Jane's iPhone" to
  // "Janes-iPhone.coredevice.local".
 ...
 * from `tmp/vibe-ios-qa.token` after the StateServer starts.
 */
export function copyFileFromAppContainer(opts: {
  ...
  const spawn = opts.spawn ?? defaultSpawn;
  const dir = mkdtempSync(join(tmpdir(), 'vibe-ios-copy-'));
