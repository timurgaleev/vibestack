// CLI tests for vibe-ios-qa-mint. Invokes the bash launcher end-to-end
...
// so the user's real ~/.vibestack/ios-qa-allowlist.json is untouched.
...
// vibestack layout: daemon nests at skills/ios-qa/daemon, launchers at skills/ios-qa/bin
const ROOT = join(import.meta.dir, '..', '..');
const MINT_BIN = join(ROOT, 'bin', 'vibe-ios-qa-mint');
const DAEMON_BIN = join(ROOT, 'bin', 'vibe-ios-qa-daemon');

function runMint(args: string[]) {
  return spawnSync(MINT_BIN, args, { stdio: 'pipe', encoding: 'utf-8' });
}

describe('bin/vibe-ios-qa-mint launcher', () => {
  let tmpDir: string;
  let listPath: string;
  ...
  test('--help prints usage without touching allowlist', () => {
    ...
    expect(r.stdout).toContain('vibe-ios-qa-mint');
...
describe('bin/vibe-ios-qa-daemon launcher', () => {
