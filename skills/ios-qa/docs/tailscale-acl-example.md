...
- **Auth:** Tailscale identity validation via the local `tailscaled` socket
  ...
  `~/.vibestack/ios-qa-allowlist.json` is the single source of truth for who can
  do what.
...
vibe-ios-qa-mint --remote ci@example.com --capability mutate --ttl 1h
...
row to `~/.vibestack/security/ios-qa-audit.jsonl`:
...
allowlisted, rate limit hit) write to `~/.vibestack/security/attempts.jsonl`.
...
| `403 identity_not_allowed` | identity missing from allowlist | Owner mint: `vibe-ios-qa-mint --remote <identity>` |
