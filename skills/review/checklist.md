...
- Options presented with only human-team effort estimates — should show both human and Claude Code + vibestack time
...
- A gap the diff itself acknowledges as accepted debt and backs by a decision recorded in the ledger — that is a call someone made with a rationale, not a Completeness Gap. **Verify before honoring it:** search the ledger for the decision itself, not for an id — `~/.vibestack/bin/vibe-decision-search --query "<a distinctive phrase from the claimed decision>"` matches on decision and rationale text, and never on the id, so querying an id always comes back empty and would suppress nothing. Read the returned entries and judge whether one actually covers this gap. Nothing matching, or nothing on point, means the comment in the diff is a justification someone wrote rather than a decision anyone recorded — flag the gap normally and say so.
