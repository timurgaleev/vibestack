...
- Removed references to an external source project from the tracked docs, skill
  bodies, changelog, and tooling — vibestack is presented as its own work. The
  parity-audit tool and its config key now speak of a neutral "reference source".
  (The SOCKS-proxy `Upstream*` type names, a networking term, were renamed to
  `Reference*` for the same reason; git's own "upstream tracking" wording in
  `/ship` is left intact.)
